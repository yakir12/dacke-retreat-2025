...
# file = "Scarabaeus_ambiguus_Boheman,_1857.jpg"
# img = reverse(FileIO.load(file)', dims = 1)
...
# sectorc = map(θ, ϵ) do θ, ϵ
#     xyz = Point3f.(CartesianFromSpherical().(Spherical.(1, range(ϵ/2, 2π - ϵ/2, 101), 0)))
#     rot = LinearMap(RotY(-θ))
#     rot.(xyz)
# end
...
# lines!(ax, sectorc, color = :gray)
lines!(ax, shadow, color = :red)

# t = text!(ax, @lift($sector[6]), text = @lift(string($ϵdeg, " °")), color = :black, align = (:center, :center), markerspace = :data, fontsize = 0.1, transform_marker = true, overdraw = true)
